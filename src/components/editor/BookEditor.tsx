...
import { LogOut, Menu, ChevronLeft, ChevronRight, Images } from 'lucide-react';
...
export default function BookEditor({ book, chapters: initialChapters, pin, onExit }: BookEditorProps) {
  ...
  const loadPages = useCallback(async (chapterId: number) => {
    ...
    if (error) { console.error('Failed to load pages:', error); return; }
    if (data) {
      const sorted = sortPagesForDisplay(data);
      setPagesByChapter((prev) => { const next = new Map(prev); next.set(chapterId, sorted); return next; });
    }
  }, []);
  ...
  useEffect(() => {
    if (current.kind === 'chapter-title' || current.kind === 'page') {
      ...
      if (chapter && !pagesByChapter.has(chapter.id)) void loadPages(chapter.id);
    }
    if (current.kind === 'chapter-title') {
      const chapter = chapters[current.chapterIndex];
      if (chapter && !galleryByChapter.has(chapter.id)) void loadChapterGallery(chapter.id);
    }
    if (current.kind === 'page') {
      const chapter = chapters[current.chapterIndex];
      const pages = pagesByChapter.get(chapter?.id) ?? [];
      const page = pages[current.pageIndex];
      if (page && !galleryByPage.has(page.id)) void loadGallery(page.id);
    }
  ...
  const logRevision = useCallback(async (params: {
    page_id?: number; chapter_id?: number; book_id?: number;
    field: string; previous_value?: string; new_value?: string;
  }) => {
    try { await supabase.from('page_revisions').insert({ ...params, edit_pin: pin }); }
    catch (err) { console.error('Failed to log revision:', err); }
  }, [pin]);
  ...
  const persistDirty = useCallback(async (payload: DirtyPayload) => {
    ...
    if (payload.kind === 'book') {
      ...
      for (const [field, value] of Object.entries(payload.changes))
        void logRevision({ book_id: book.id, field, new_value: value == null ? undefined : String(value) });
    } else if (payload.kind === 'chapter') {
      const { error } = await supabase.from('chapters').update(payload.changes).eq('id', payload.chapterId);
      if (error) throw error;
      for (const [field, value] of Object.entries(payload.changes))
        void logRevision({ chapter_id: payload.chapterId, book_id: book.id, field, new_value: value == null ? undefined : String(value) });
    } else {
      const { error } = await supabase.from('pages').update(payload.changes).eq('id', payload.pageId);
      if (error) throw error;
      for (const [field, value] of Object.entries(payload.changes))
        void logRevision({ page_id: payload.pageId, chapter_id: payload.chapterId, book_id: book.id, field, new_value: value == null ? undefined : String(value) });
    }
  }, [book.id, logRevision]);

  const autosave = useAutosave({
    value: dirty,
    onSave: async (val) => { if (!val) return; await persistDirty(val); setDirty(null); },
    delay: 1500,
  ...
  const handleAddToGallery = useCallback(async (
    ...
    const nextOrder = existing.length > 0 ? Math.max(...existing.map((g) => g.sort_order ?? 0)) + 1 : 0;
    const { error } = await supabase.from('gallery').insert({
      image_url: imageUrl, image_caption: caption, chapter_id: chapterId, page_id: pageId, sort_order: nextOrder,
    });
    if (error) { console.error('Failed to add to gallery:', error); return; }
    ...
    if (current.kind === 'page') {
      ...
      if (chapter?.id === chapterId) {
        ...
        if (deletedIndex === current.pageIndex) {
          if (remaining.length > 0) setCurrent({ kind: 'page', chapterIndex: current.chapterIndex, pageIndex: Math.min(current.pageIndex, remaining.length - 1) });
          else setCurrent({ kind: 'chapter-title', chapterIndex: current.chapterIndex });
        } else if (deletedIndex < current.pageIndex) {
          setCurrent({ kind: 'page', chapterIndex: current.chapterIndex, pageIndex: current.pageIndex - 1 });
        }
      }
    }
  }, [pagesByChapter, dirty, autosave, current, chapters, book.id, logRevision]);

  const handleReorderPages = useCallback(async (chapterId: number, fromIndex: number, toIndex: number) => {
    ...
    try {
      ...
      setPagesByChapter((prev) => { const next = new Map(prev); next.set(chapterId, existing); return next; });
    ...
    if (current.kind === 'page') {
      ...
      if (chapter?.id === chapterId) {
        ...
        if (newIdx >= 0 && newIdx !== current.pageIndex) setCurrent({ kind: 'page', chapterIndex: current.chapterIndex, pageIndex: newIdx });
      }
    }
  }, [pagesByChapter, current, chapters, book.id, logRevision]);

  const handleExit = async () => { await autosave.flush(); onExit(); };

  const view = renderCurrentView();

  function renderCurrentView(): React.ReactNode {
    if (current.kind === 'cover') return <CoverEditView book={bookState} onChange={updateBook} />;
    if (current.kind === 'dedication') return <DedicationEditView book={bookState} onChange={updateBook} />;
    if (current.kind === 'intro') return <IntroEditView book={bookState} onChange={updateBook} />;
    if (current.kind === 'chapter-title') {
      const chapter = chapters[current.chapterIndex];
  ...
  function renderBreadcrumb(): React.ReactNode {
    if (current.kind === 'intro') return <BreadcrumbPill>Introduction</BreadcrumbPill>;
    if (current.kind === 'page') {
      const chapter = chapters[current.chapterIndex];
      const pages = pagesByChapter.get(chapter?.id) ?? [];
  ...
  const goPrev = () => { const p = prevState(current, allStates); if (p) setCurrent(p); };
  const goNext = () => { const n = nextState(current, allStates); if (n) setCurrent(n); };
        ...
        <div className="max-w-full px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 md:gap-3 shrink-0">
            ...
            <a
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-sm font-avenir text-amber-700 border border-amber-200 bg-amber-50 rounded-full hover:bg-amber-100 transition-colors"
            >
