...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        // Top-level: configurable from props. Figure captions: hardcoded.
        placeholder: ({ node }) => {
          if (node.type.name === 'figure') return 'Add a caption…';
      ...
      // Image extension still registered so standalone <img> tags from legacy
      // WordPress content render. Our Figure extension takes priority (60 vs
      // default 50) for <figure> matches.
      Image.configure({
        HTMLAttributes: { class: 'rounded-lg my-4' },
        allowBase64: false,
      }),
      // The Figure node — bookSlug is needed by the NodeView for re-uploads
  ...
  // Re-sync external value changes
  useEffect(() => {
    if (!editor) return;
  ...
  /**
   * Inserts an inline figure using the Figure node's custom command.
   * The command places a paragraph after so the caret has somewhere to land.
   */
